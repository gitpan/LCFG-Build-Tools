package LCFG::Build::Utils;    # -*-cperl-*-
use strict;
use warnings;

# $Id: Utils.pm.in,v 1.9 2008/09/12 14:05:30 squinney Exp $
# $Source: /disk/cvs/dice/LCFG-Build-Tools/lib/LCFG/Build/Utils.pm.in,v $
# $Revision: 1.9 $
# $HeadURL$
...
{
    my $datadir = exists $ENV{LCFG_BUILD_TMPLDIR}
...
{    # Caching going on here, make variables only readable by this method.
    ...
    sub load_configs {
        ...
        my $lcfg_file = File::Spec->catfile( $datadir, 'lcfg_config.yml' );
        my $map_file  = File::Spec->catfile( $datadir, 'mapping_config.yml' );

        $lcfgcfg ||= YAML::Syck::LoadFile($lcfg_file);
        $mapping ||= YAML::Syck::LoadFile($map_file);
...
sub generate_cmake {
    ...
    my $datadir = datadir();
    my $tmpldir = File::Spec->catdir( $datadir, 'templates' );
...
=head1 VERSION
    ...
    This documentation refers to LCFG::Build::Utils version 0.0.45
